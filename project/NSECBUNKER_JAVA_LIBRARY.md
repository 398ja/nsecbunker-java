...
| Task ID | Task Description | Note | Size | Commit ID | Dependency | Status |
|---------|-----------------|------|------|-----------|------------|--------|
| 1.2.1 | Implement `BunkerConnection` model | Connection string representation | S | | 1.1.1 | ⬜ TODO |
| 1.2.2 | Implement `BunkerKey` model | Key metadata (name, npub, counts) | S | | 1.1.1 | ⬜ TODO |
| 1.2.3 | Implement `BunkerPolicy` model | Policy with rules list | S | | 1.1.1 | ⬜ TODO |
| 1.2.4 | Implement `PolicyRule` model | Method, kind, usage limits | S | | 1.2.3 | ⬜ TODO |
| 1.2.5 | Implement `KeyUser` model | User pubkey, permissions, metadata | S | | 1.1.1 | ⬜ TODO |
| 1.2.6 | Implement `SigningCondition` model | ACL rules per key user | S | | 1.2.5 | ⬜ TODO |
| 1.2.7 | Implement `AccessToken` model | Token with policy, expiration | S | | 1.2.3 | ⬜ TODO |
| 1.2.8 | Create `BunkerException` hierarchy | Typed exceptions for errors | S | | 1.1.1 | ⬜ TODO |

#### 1.3 Connection Management (`nsecbunker-core/connection`)

| Task ID | Task Description | Note | Size | Commit ID | Dependency | Status |
|---------|-----------------|------|------|-----------|------------|--------|
| 1.3.1 | Implement `BunkerConnectionString` parser | Parse bunker:// URIs | M | | 1.2.1 | ⬜ TODO |
...
| Task ID | Task Description | Note | Size | Commit ID | Dependency | Status |
|---------|-----------------|------|------|-----------|------------|--------|
| 1.5.1 | Create mock relay server | For testing without real relays | L | | 1.3.2 | ⬜ TODO |
...
| Task ID | Task Description | Note | Size | Commit ID | Dependency | Status |
|---------|-----------------|------|------|-----------|------------|--------|
| 2.1.1 | Implement `NsecBunkerAdminClient` class | Main admin client with builder | M | | Phase 1 | ⬜ TODO |
| 2.1.2 | Add admin connection setup | Generate ephemeral keypair for auth | S | | 2.1.1 | ⬜ TODO |
| 2.1.3 | Implement admin request/response handling | Send kind 24134 messages | M | | 2.1.1, 1.4.* | ⬜ TODO |
| 2.1.4 | Add admin authentication validation | Verify admin npub matches | S | | 2.1.3 | ⬜ TODO |
...
| 2.6.2 | Create key CRUD operation tests | Create, list, unlock, delete | M | | 2.2.* | ⬜ TODO |
| 2.6.3 | Create policy CRUD operation tests | Create, list, get, delete | M | | 2.3.* | ⬜ TODO |
| 2.6.4 | Create permission grant/revoke tests | Grant, revoke, list | M | | 2.4.* | ⬜ TODO |
| 2.6.5 | Create token creation/validation tests | Create, validate, revoke | M | | 2.5.* | ⬜ TODO |
...
| Task ID | Task Description | Note | Size | Commit ID | Dependency | Status |
|---------|-----------------|------|------|-----------|------------|--------|
| 3.6.1 | Create adapter for `IIdentity` interface | nostr-java compatibility | M | | 3.1.1 | ⬜ TODO |
| 3.6.2 | Implement compatibility layer | Bridge differences | M | | 3.6.1 | ⬜ TODO |
| 3.6.3 | Add event signing integration | Work with nostr-java events | S | | 3.6.2, 3.2.1 | ⬜ TODO |
